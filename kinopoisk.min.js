...
$kp_rating.stars=el.kinopoisk("_getStar",$kp_rating.text(),settings);$imdb_rating.stars=el.kinopoisk("_getStar",$imdb_rating.text(),settings);var ratings={"kinopoisk":el.kinopoisk("_getTemplate",settings.kinopoisk_template,$kp_rating,settings),"imdb":el.kinopoisk("_getTemplate",settings.imdb_template,$imdb_rating,settings)};var text="";for(var i in settings.order){text+=ratings[settings.order[i]];}
return el.hide().html('<span class="kp_container">'+text+'</span>').fadeIn();},_getTemplate:function(template,$rating,settings){return template.replace("$rating",$rating.text()).replace("$vote",$rating.attr("num_vote")).replace("$stars",$rating.stars);},_getStar:function(rating,settings){var star="";var round_rating=Math.round(rating*settings.range/10);for(var i=1;i<=settings.range;i++){if(i<=round_rating){star+="<span>&#9733;</span>";}else{star+="<span>&#9734;</span>";}}
