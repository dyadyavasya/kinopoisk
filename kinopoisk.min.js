...
$.ajax({type:'GET',url:'http://query.yahooapis.com/v1/public/yql?q='
+encodeURIComponent('select * from xml where url="'+params.url+'/'+params.movie
+'.xml"')+'&format=xml&callback=?',dataType:'json',success:function(data){return el.kinopoisk("_showRating",data);},error:function(data){console.log(data);$.error(data.responseText);}});},_showRating:function(data){var el=$(this);var params=el.data('params');if(!data.results[0]){throw'Проверьте правильность url "'+params.url+'"';}
var xml_doc=$.parseXML(data.results[0]);var $xml=$(xml_doc);var $kp_rating=$xml.find("kp_rating");var $imdb_rating=$xml.find("imdb_rating");if($kp_rating.text()==0&&$kp_rating.attr("num_vote")==0){return el.html('<span class="kp_container">Нет данных</span>');}
$kp_rating.stars=el.kinopoisk("_getStar",$kp_rating.text(),params);$imdb_rating.stars=el.kinopoisk("_getStar",$imdb_rating.text(),params);var ratings={"kinopoisk":el.kinopoisk("_getTemplate",params.kinopoisk_template,$kp_rating),"imdb":el.kinopoisk("_getTemplate",params.imdb_template,$imdb_rating)};var text="";for(var i in params.order){text+=ratings[params.order[i]];}
return el.hide().html('<span class="kp_container">'+text+'</span>').fadeIn();},_getTemplate:function(template,$rating){return template.replace("$rating",$rating.text()).replace("$vote",$rating.attr("num_vote")).replace("$stars",$rating.stars);},_getStar:function(rating,params){var star="";var round_rating=Math.round(rating*params.range/10);for(var i=1;i<=params.range;i++){if(i<=round_rating){star+="<span>&#9733;</span>";}else{star+="<span>&#9734;</span>";}}
